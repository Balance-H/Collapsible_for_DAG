...
static igraph_error_t dag_to_undirected_moral_graph(
    const igraph_t *graph,
    igraph_t *ug_graph
) {
    if (!graph || !ug_graph) {
    ...
    const igraph_integer_t n = igraph_vcount(graph);
    igraph_error_t ret = igraph_empty(ug_graph, n, IGRAPH_UNDIRECTED);
    if (ret != IGRAPH_SUCCESS) {
        return ret;
    }
    ...
    for (igraph_integer_t v = 0; v < n; ++v) {
        igraph_vector_int_t *parent_list = igraph_adjlist_get(&parents, v);
        ...
        for (igraph_integer_t i = 0; i < parent_count; ++i) {
            ...
            for (igraph_integer_t j = i + 1; j < parent_count; ++j) {
                ...
                ret = igraph_are_adjacent(ug_graph, a, b, &adjacent);
                if (ret != IGRAPH_SUCCESS) {
                    igraph_adjlist_destroy(&children);
                    igraph_adjlist_destroy(&parents);
                    return ret;
                }

                if (!adjacent) {
                    ret = igraph_add_edge(ug_graph, a, b);
                    if (ret != IGRAPH_SUCCESS) {
                        igraph_adjlist_destroy(&children);
                        igraph_adjlist_destroy(&parents);
                        return ret;
    ...
    ret = igraph_get_edgelist(graph, &edge_list, 0);
    if (ret != IGRAPH_SUCCESS) {
    ...
    for (igraph_integer_t ei = 0; ei < ecount; ++ei) {
        ...
        igraph_bool_t adjacent = 0;
        ret = igraph_are_adjacent(ug_graph, u, v, &adjacent);
        if (ret != IGRAPH_SUCCESS) {
            igraph_vector_int_destroy(&edge_list);
            igraph_adjlist_destroy(&children);
            igraph_adjlist_destroy(&parents);
        ...
        if (!adjacent) {
            ret = igraph_add_edge(ug_graph, u, v);
            if (ret != IGRAPH_SUCCESS) {
                igraph_vector_int_destroy(&edge_list);
                igraph_adjlist_destroy(&children);
                igraph_adjlist_destroy(&parents);
