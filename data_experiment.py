...
def run_experiment(r_repeat, sample_repeat, sample_sizes, r_size, file_names, seed=42):
    if r_repeat <= 0:
    ...
    for file in file_names:
        ...
        # 为当前网络预先固定 r_repeat 组 R，后续所有样本量复用同一组 R。
        fixed_r_groups = [random.sample(node_names, r_size) for _ in range(r_repeat)]

        # 对固定 R 预计算真分布和索引，避免在不同样本量下重复计算。
        inference_true = VariableElimination(true_model)
        fixed_r_contexts = []
        for r_names in fixed_r_groups:
            true_query = inference_true.query(variables=r_names, evidence={}, show_progress=False)
            r_idx = [node_to_idx[node] for node in r_names]
            fixed_r_contexts.append((r_names, r_idx, true_query))

        for sample_size in sample_sizes:
            r_case_rows = []

            for r_names, r_idx, true_query in fixed_r_contexts:
                ...
                for _ in range(sample_repeat):
                    ...
                    # 使用完整数据：不制造缺失，直接用对应变量子集建模。
                    ang_df_full = df[ang_nodes]
                    collapsed_df_full = df[collapsed_nodes]
            ...
            """
            print(
                f"file={file}, sample_size={sample_size}, "
...
def main():
    ...
    result.to_csv(result_dir / "sample_size_experiment.csv", index=False)
    print(result.head())
